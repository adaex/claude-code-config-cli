...
- **别名映射**（如 openrouter-2o）：model_name 用 `claude-opus-4-7` / `claude-sonnet-4-6`，model 用上游名。适合需要伪装为 Claude 模型的场景。
...
代理名跟随 Opus 大版本递增：opus-4-7 → `openrouter-2o`，opus-4-8 → `openrouter-3o`。haiku slot 用于放上一代 opus 模型。

当前映射（openrouter-2o）：
- opus → `claude-opus-4-7`（上游 openrouter-2o）
- sonnet → `claude-sonnet-4-6`（上游 openrouter-1）
- haiku → `claude-opus-4-6`（上游 openrouter-1o，即上一代 opus）

升级到下一代（如 opus-4-8）时需：

1. 重命名代理目录 `~/.ccc/proxies/openrouter-2o` → `openrouter-3o`
2. 更新 `config.yaml` — opus 指向新上游模型，haiku 指向当前 openrouter-2o
3. `~/.ccc/claude.zsh` — `_cc-ensure` 代理名、`ANTHROPIC_DEFAULT_OPUS_MODEL` 更新
4. `CLAUDE.md` — 端口分配表、本节示例更新
5. 重启代理：`ccc proxy stop openrouter-2o && ccc proxy start openrouter-3o`
